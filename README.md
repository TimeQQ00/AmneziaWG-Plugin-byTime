...
**Автор:** Time · **Версия:** 1.2.0

---

## Возможности

- 📄 **Свои конфиги** — импорт `.conf` (AmneziaWG/WireGuard) в два тапа:
  кнопка ищет файлы в Download/Documents и предлагает выбор, есть импорт из
  буфера обмена и ручной путь. Конфиг запоминается и используется при каждом
  старте (и **никогда не стирается автоматически**).
...
3. Плагин попросит импортировать `.conf` — положите файл конфига в Download
   (или Documents) и в настройках плагина нажмите **«Выбрать .conf на устройстве»**.
...
2. Настройки плагина → **«Выбрать .conf на устройстве»** (авто-поиск в
   Download/Documents), или **«Импорт из буфера обмена»** (скопируйте текст
   конфига), или старый добрый **«Путь к .conf файлу»** → «Импортировать».
